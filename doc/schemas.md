...
리포트는 두 집계 함수에서 핵심 숫자를 뽑는다 — `report/md_report.py::_device_aggregates` (device CSV → op별 stats) + `_system_aggregates` (system CSV → node/iface별 stats). 공통 CSV/topology 로더는 `report/datasource.py`. md/summary/png/pdf 리포트가 전부 이 두 dict를 소비한다.

### 메트릭 매핑 표

| 메트릭 | 집계 방법 | 입력 CSV 컬럼 | CSV 생성 위치 | 원본 데이터 출처 | 경고 임계 |
|---|---|---|---|---|---|
| **Read IOPS (total)** | Σ devices × op∈{read, read_ahead} : `iops.sum` | `iops_interval` | `monitoring/collectors/ebpf_io/collector.py::parse_and_store_metrics` | BPF `block_rq_complete` → `device_stats[dev].stats[IO_READ].io_count` 인터벌 delta | — |
| **Write IOPS (total)** | Σ devices : `iops.sum` (op=write) | `iops_interval` (op=write 행) | 위와 동일 | 위와 동일 (`type=IO_WRITE`) | — |
| **Peak BW** | max(devices × ops : `bw.max`) | `bandwidth_mb_s_interval` | 위와 동일 | BPF `target->total_bytes` 인터벌 delta ÷ elapsed | — |
| **Worst D2C interval avg** | max(devices × ops : `d2c.max`) | `d2c_avg_us_interval` | 위와 동일 | BPF `block_rq_issue→block_rq_complete` 시간차 (`target->d2c.total / count`) per interval | `>500us` → warn |
| **SQ↔CQ diff** | mean(devices : `_sqcq_diff_ratio.avg`) | `sq_cq_diff_ratio` | 위와 동일 | BPF `block_rq_complete`에서 `bpf_get_smp_processor_id()` vs `trace_ctx.issue_cpu` 비교 → `device_qd.sq_cq_{same,diff}` 누적의 인터벌 delta 비율 | `>20%` bad, `>5%` warn |
| **CPU iowait peak** | max(nodes : `iowait_pct.max`) | `node{N}_iowait_pct` | `monitoring/collectors/system.py::_tick` | `/proc/stat` 의 per-CPU iowait jiffies 인터벌 delta → NUMA node 단위 합산 / total × 100 | `>10%` bad, `>2%` warn |
| **CPU sys peak** | max(nodes : `sys_pct.max`) | `node{N}_sys_pct` | 위와 동일 | `/proc/stat` sys jiffies | `>50%` warn |
| **GPU power peak** *(있을 때만)* | max(gpus : `pwr_w.max`) | `gpu{N}_pwr_w` | `monitoring/collectors/system.py::_gpu_reader_loop` (background thread) | `nvidia-smi dmon -s pumt` stream의 `pwr` 컬럼 | `>50W` warn |

### "max"의 정확한 의미

`_stats(values)` 헬퍼가 인터벌 값 리스트(예: 7개의 1초 평균)에서 `{n, avg, min, max, sum}` 을 계산. 즉 Peak BW·Worst D2C의 "max"는 **인터벌별 평균 중 가장 큰 인터벌** — 전체 누적 평균이 아닌 **가장 격렬했던 1초**.

예) "Worst D2C interval avg" 값이 7700us = "어떤 1초 동안 d2c 평균이 7.7ms였다". 통상 첫 인터벌(preconditioning) 또는 GC 트리거 시 발생.

### 한 데이터 흐름으로 본 예시 — Worst D2C
...
fio randread → 커널 block layer
  ...
  ↓ md/summary 리포트: dev_aggs[d][op]["d2c"]["max"]
```

### Top findings

`md_report._top_findings(sys_agg, dev_aggs)` 의 heuristic 결과:
...
임계값은 `_top_findings` 안의 inline 조건문에서 직접 수정한다.

## 6. 리포트 파일

- `report_<sid>.md`: Top findings + Topology + Device aggregate + System aggregate 요약 (~1.5KB 포터블 텍스트).
- `summary_<sid>.json`: 단일 평탄화 JSON export (프로그램적 소비용, 스키마 stable).
- `report_png_<sid>.md` + `figs_<sid>/*.png`: 정적 PNG 차트 + 이를 참조하는 markdown.
- `report_<sid>.pdf`: 표지(markdown 구조 렌더) + PNG 차트 페이지를 묶은 단일 PDF.
