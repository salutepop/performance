...
class Scenario:
    def __init__(self):
        self.tc_name = "TC05_NUMA_Core_Min_Latency"
        self.description = "시스템의 모든 CPU 코어를 순회하며 QD1 환경에서 단일 코어 최소 지연시간을 측정합니다."
    ...
    def execute(self, disk, runner_func, reporter, session_dir, numa_node):
        ...
        for core in range(total_cores):
            wl = {
                "name": f"core_{core}_qd1_randread",
                "rw": "randread",
                "bs": "4k",
                "iodepth": 1,  # 단일 코어 최대 성능을 뽑기 위한 QD 설정
                "numjobs": 1,  # 1개 Job으로 한정하여 코어 성능 집중
                "runtime": 1,  # 안정적인 측정을 위해 10초로 증설 (기존 1초는 너무 짧음)
                "time_based": 1,
    ...
    def _print_combined_report(self):
        ...
        print("  ▶ [통합 결과] CPU 코어별 단일 코어 최소 지연시간 리포트 (QD1)")
        print(
            "  ======================================================================="
        )
